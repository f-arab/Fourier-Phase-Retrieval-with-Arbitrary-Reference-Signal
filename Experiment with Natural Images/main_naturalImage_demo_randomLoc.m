...
for im = 1: length(image_name)
    ...
    known_percent = [40 35 30 25 20 15 10 5 1];          
    rng(0)
...
% save('natural_images_removedNormalization_randLoc_zeroInitial_lambda_5000','x_est','measurement_err_2','known_percent','psnr_all','ssim_all')
%%
fig = figure(300); fig.Position = [100 250 1550 650];
[ha, pos] = tight_subplot(size(x_est,1), size(x_est,2)-2, [0.05 0.01], [0.05 0.05], [0.01 0.01]);

k = 1;
for h = 1:length(image_name)
    for v = 1:size(x_est,2)-2
        axes(ha(k))
