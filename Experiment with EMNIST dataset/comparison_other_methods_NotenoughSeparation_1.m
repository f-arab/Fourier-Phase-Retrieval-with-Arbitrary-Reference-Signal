...
xt = [x1 x2];
xt = padarray(xt,[14 0],0,'both')
 
%% First Scenario: All the methods work

known_reference = zeros(size(xt));
known_reference(10:55,end-8:end-5) = 1;
known_reference(13:16,end-11:end-5) = 1;
...
n = numel(xt_reference_added);                           % Total number of samples in the original signal
m = 4*n;
MaskPatterns = ones(size(xt_reference_added));
Image_Support = ones(size(xt_reference_added));
...
Autocorr_xt_reference_added = real(ifft2(reshape(b,2*size(xt_reference_added)).^2));
...
cross_corr_nonlinear = xcorr2(xt,xt);

% component 3: cross correlation of reference and unknown signal
cross_corr_linear = xcorr2(xt,known_reference);
...
axes(ha(1))
imagesc(xt_reference_added);colormap gray;title('Original');xticks([]);yticks([])
...
x0 = zeros(size(xt_reference_added));
x0(known_reference_support ==1) = xt_reference_added(known_reference_support ==1);
...
opts.knownReference_values = xt_reference_added(known_reference_support ==1);
[x,measurement_error] = PRGradientDescentSolver(x0,A,At,b,opts);
x = reshape(x,size(xt_reference_added));
x = x(:,1:56);
...
MR = ref2mtrx(known_reference);
Y = [eye(56) zeros(56,56)]*ifftshift(Autocorr_xt_reference_added)*[eye(56);zeros(3*56,56)];

x_recovered_candes = pinv(MR)*Y(:);
...
% original
axes(hc_1);imagesc(xt_reference_added);colormap(hc_1,'gray');xticks([]); yticks([])
...
axes(hc_8);imagesc(reshape(x_recovered_candes,size(xt)));colormap(hc_8,'gray');xticks([]); yticks([])
